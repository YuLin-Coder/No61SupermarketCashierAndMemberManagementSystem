...
<html xmlns="http://www.w3.org/1999/xhtml">
				...
				<td>
					<table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" bgcolor="#aec3de">
						<tr align="left" bgcolor="#F2FDFF">
							<td height="30" colspan="17" background="images/table_header.gif">购买结账列表</td>
						</tr>
					</table>
				</td>
			</tr>
  ...
  %>>
                             ...
                             <td>${u.shangpinbianhao}</td>    <td>${u.shangpinmingcheng}</td>    <td>${u.leibie}</td>    <td>${u.jiage}</td>    <td>${u.kucun}</td>    <td>${u.goumaishuliang}</td>    <td>${u.goumaijine}</td>    <td>${u.xingming}</td>    <td>${u.shoujihao}</td>    <td>${u.yue}</td>    
                              ...
                              <td height="50" align="center"><a class="btn btn-info btn-small" href="doUpdateGoumaijiezhang.do?id=${u.id }">编辑</a>  <a class="btn btn-info btn-small" href="deleteGoumaijiezhang.do?id=${u.id }"
...
while(RS_result.next()){
		...
		共计购买金额:<%=goumaijinez%>； 
		 ...
		 <c:if test="${page.page>1}">
              <a href="goumaijiezhangList.do?page=1" >首页</a>
             <a href="goumaijiezhangList.do?page=${page.page-1 }"> 上一页</a>             </c:if>
    	     <c:if test="${page.page<page.totalPage}">
			<a href="goumaijiezhangList.do?page=${page.page+1 }">下一页</a>
			<a href="goumaijiezhangList.do?page=${page.totalPage }">末页</a>		    </c:if>		
	</c:if>
	</p>
